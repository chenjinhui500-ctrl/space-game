...
app.innerHTML = '<canvas id="game"></canvas><div class="loading-note">WASD / 方向键移动 · 自动攻击 · 触摸左下角虚拟摇杆</div>';
...
function autoAttack() {
  const target = enemies.toSorted((a, b) => distance(player.x, player.y, a.x, a.y) - distance(player.x, player.y, b.x, b.y))[0];
  if (!target) return;
...
function rounded(x: number, y: number, w: number, h: number, color: string) { ctx.fillStyle = color; ctx.strokeStyle = '#7dd3fc'; ctx.lineWidth = 2; ctx.beginPath(); ctx.roundRect(x, y, w, h, 18); ctx.fill(); ctx.stroke(); }
